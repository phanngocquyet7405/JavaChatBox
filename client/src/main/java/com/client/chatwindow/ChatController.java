...
public class ChatController implements Initializable {
    ...
    public void setUserList(Message msg) {
        ...
        Platform.runLater(() -> {
            if (msg.getUsers() != null) {
                ...
                ObservableList<User> allUsers = FXCollections.observableList(msg.getUsers());

                CellRenderer cellRenderer = new CellRenderer(currentUserName);
                cellRenderer.updateUserList(allUsers);

                userList.setItems(cellRenderer.getUserList());
                userList.setCellFactory(cellRenderer);
                setOnlineLabel(String.valueOf(cellRenderer.getUserList().size()));
            } else {
    ...
    public void initializeMessages(int currentUserId) {
        this.currentUserId = currentUserId;
        userList.setOnMouseClicked(this::handleUserSelection);
    }

    // Khi người dùng nhấn vào một user trong ListView
    private void handleUserSelection(MouseEvent event) {
        ...
        if (selectedUser != null) {
            selectedUserId = chatService.getUserIdByUsername(selectedUser);
    ...
    private void loadChat(int otherUserId) {
        List<String> chatHistory = chatService.loadChatHistory(currentUserId, otherUserId);
        if (chatHistory != null) {
            chatPane.getItems().addAll(chatHistory);
        }
    }
